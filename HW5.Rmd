...
scatter1 <- citysum %>% filter(!is.na(Prop_DM)) %>%
  plot_ly(x = ~mean_score, y = ~Prop_DM,
          ...
          text = ~paste(paste(FACILITY_CITY),  paste(" Average score:",
                        mean_score, sep=""), paste(" Proportion DM:",
                        Prop_DM, sep=""))) %>%
  add_lines(x = ~mean_score, y=fitted(fit1)) %>%
  layout(title = "Average restaurant inspection score vs proportion of diabetes by city", yaxis = list(title = "Proportion Diabetes"), xaxis = list(title = "Average Inspection Score"), hovermode = "compare") %>%
  layout(showlegend = F)
...
scatter2 <- citysum %>% filter(!is.na(Prop_obse)) %>%
  plot_ly(x = ~mean_score, y = ~Prop_obse,
          ...
          add_markers( hoverinfo = 'text',
          text = ~paste(paste(FACILITY_CITY),  paste(" Average score:",
                        mean_score, sep=""), paste(" Proportion Obese:",
                        Prop_obse, sep=""))) %>%
  ...
  layout(plot_bgcolor='#e5ecf6', showlegend = F)
...
scatter3 <- citysum %>% filter(!is.na(Prop_depr)) %>%
  plot_ly(x = ~mean_score, y = ~Prop_depr,
          ...
          add_markers( hoverinfo = 'text',
          text = ~paste(paste(FACILITY_CITY),  paste(" Average score:",
                        mean_score, sep=""), paste(" Proportion with depression:",
                        Prop_depr, sep=""))) %>%
