...
score.pal <- colorNumeric("inferno", domain=chains$SCORE)
DM.pal <- colorNumeric("viridis", domain=chains$Prop_DM)
DMmap <- leaflet(lamap) %>% addTiles() %>%
  addPolygons(data=chains,
              color = ~DM.pal(Prop_DM)) %>%
  addCircles(
    lat = ~chains$latitude, lng = ~chains$longitude,
    label = ~paste0(chains$SCORE), color = ~score.pal(chains$SCORE),
