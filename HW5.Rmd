...
zips <- unique(chains$FACILITY_ZIP)e 
...
scoremap <- leaflet(chains) %>%
  ...
  addCircles(
    ...
    label = ~paste0(paste(FACILITY_NAME), ", ", paste0(round(chains$avgscore), 2)), color = ~score.pal(avgscore),
...
Obmap <- leaflet(lamap) %>% addTiles() %>%
  ...
  addCircles(
   ...
   label = ~paste0(paste(chains$FACILITY_NAME), ", ", paste0(round(chains$avgscore), 2)), color = ~score.pal(chains$avgscore),
